...
* Disciplinas: 
    + Modelos de Regressão (CE071 - 2014/1S)
...
* Sugestão de leitura do artigo *The Gamma-count distribution in the analysis of
  experimental underdispersed data* por @Zeviani2014:
      ...
      + Discussão de aspectos inferênciais
...
* Número de insetos mortos após $k$ dias da aplicação de inseticidade;
* ...
...
\begin{center}
\textcolor{red}{Muitas vezes a suposição de equidispersão não é atendida}
\end{center}
...
<!-- \begin{equation*} -->
<!--     \begin{split} -->
<!--         Y \mid &X \sim \text{COM-Poisson}(\lambda_i, \nu) \\ -->
<!--         &g(\lambda_i) = X \underline{\beta} -->
<!--     \end{split} -->
<!-- \end{equation*} -->
<!-- \end{block} -->
...
xyplot(py.com ~ c(yy - 0.12) | caso, type = c("h", "g"),
       ...
       strip = strip.custom(bg = "gray90",
                            factor.levels = fl)) + 
  as.layer(xyplot(py.pois ~ c(yy + 0.12) | caso, 
                  lwd = 2.5, col = cols[1],
                  type = "h"))
for(i in 1:3){
...
```{r defol, fig.height = 5, fig.width = 8}
library(legTools)
defol <- defoliation
...
Quando há uma estrutura de covariância entre as obervações induzida pelo delineamento
experimental ou por um processo hierárquico.
